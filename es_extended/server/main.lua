...
function onPlayerJoined(playerId)
	...
	if identifier then
		if ESX.GetPlayerFromIdentifier(identifier) then
			DropPlayer(playerId, ('Il y a eu une erreur dans le chargement de votre personnage.\nCode d\'erreur: identifier-active-ingame\n\nCette erreur peut être causée par un autre joueur qui a le même identifiant que vous. Vérifiez que vous ne jouez pas sur le même compte Rockstar\n\nVotre identifiant Rockstar: %s'):format(identifier))
		else
		...
		DropPlayer(playerId, 'Il y a eu une erreur dans le chargement de votre personnage.\nCode d\'erreur: identifier-missing-ingame\n\nErreur inconnue : votre identifiant est introuvable. Veuillez réessayer plus tard ou faire un ticket sur le discord.')
	end
end
...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
	...
	if identifier then
		if ESX.GetPlayerFromIdentifier(identifier) then
			deferrals.done(('Il y a eu une erreur dans le chargement de votre personnage.\nCode d\'erreur: identifier-active-ingame\n\nCette erreur peut être causée par un autre joueur qui a le même identifiant que vous. Vérifiez que vous ne jouez pas sur le même compte Rockstar\n\nVotre identifiant Rockstar: %s'):format(identifier))
		else
			deferrals.done()
		end
	else
...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, job2, job2_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
			...
			if ESX.DoesJobExist(job, grade) then
				...
				print(('[es_extended] [^3WARNING^7] Métier invalide pour %s [job: %s, grade: %s]'):format(identifier, job, grade))
			...
			if ESX.DoesJob2Exist(job2, grade2) then
				...
				print(('[es_extended] [^3WARNING^7] Job2 invalide pour %s [job2: %s, grade2: %s]'):format(identifier, job2, grade2))
			...
			if result[1].inventory and result[1].inventory ~= '' then
				...
				for name,count in pairs(inventory) do
					...
					if item then
						...
						print(('[es_extended] [^3WARNING^7] Objet invalide "%s" pour "%s"'):format(name, identifier))
					end
				end
			end
			...
			if result[1].position and result[1].position ~= '' then
				...
				print('[es_extended] [^3WARNING^7] La colonne "position" de la table "users" nécessite une valeur par défaut utilisant des coordonnées de backup.')
	...
	Async.parallel(tasks, function(results)
		...
		print(('[es_extended] [^2INFO^7] Le joueur "%s^7" s\'est connecté, id attribué : %s'):format(xPlayer.getName(), playerId))
	end)
end
