...
ESX.SavePlayer = function(xPlayer, cb)
	...
	Async.parallel(asyncTasks, function(results)
		print(('[^2INFO^7] Joueur sauvegardé : "%s^7"'):format(xPlayer.getName()))
...
ESX.SavePlayers = function(cb)
	...
	for i=1, #xPlayers, 1 do
		table.insert(asyncTasks, function(cb2)
			...
			TriggerClientEvent('esx:showColoredNotification', xPlayer.source, "~g~Personnage \n~s~Synchronisation effectuée. ", 20) -- ~ vert 140 si on veut noir
		end)
	end

	Async.parallelLimit(asyncTasks, 8, function(results)
		print(('[^2INFO^7] %s joueur(s) sauvegardé(s)'):format(#xPlayers))
		if cb then
			cb()
		end
	end)
